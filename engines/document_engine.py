...
class DocumentEngine:
    ...
    def process(file_path: str | Path) -> EngineResult:
        ...
        for af in raw.get("ambiguous_fields", []):
            result.ambiguous_fields.append(AmbiguousField(
                ...
                confidence=float(scores.get(af.get("field", ""), 0.5)),
        ...
        elif overall_confidence >= CONFIDENCE_WARN_PROCEED:
            result.add_warning(
                f"Confidence {overall_confidence:.2f} — proceeding with warnings"
            )
        elif overall_confidence >= CONFIDENCE_HUMAN_REVIEW:
            result.flag_for_review(
                f"Confidence {overall_confidence:.2f} is below auto-proceed threshold"
            )
        else:
            result.flag_for_review(
                f"LOW CONFIDENCE {overall_confidence:.2f} — high priority review required"
            )
    ...
    def _build_document(raw: dict) -> ExtractedDocument:
        ...
        contract = Contract(
            contract_id=con_data.get("contract_id") or "UNKNOWN",
            ...
            gst_rate=float(con_data.get("gst_rate") or 0.18),
